...
//		line(x_0, y_0, x_2, y_2, image, white);
		 int norm_x=(y_1-y_0)*(z_2-z_0)-(y_2-y_0)*(z_1-z_0);
	     int norm_y=(x_2-x_0)*(z_1-z_0)-(x_1-x_0)*(z_2-z_0);
	     int norm_z=(x_1-x_0)*(y_2-y_0)-(x_2-x_0)*(y_1-y_0);

	     int light_x=-1;
	     int light_y=-1;
	     int light_z=-1;

	     float cos_alf = (light_x*norm_x+light_y*norm_y+light_z*norm_z)/sqrt3(norm_x,norm_y,norm_z)/sqrt3(light_x,light_y,light_z);

	     int color_code = (int)((cos_alf+1.0)/2*255);
		color_triangle(x_0, y_0, x_1,y_1, x_2, y_2, image, TGAColor(color_code,color_code, color_code, 255));
